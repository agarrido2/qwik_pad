  ...
  "rounded-lg bg-card text-card-foreground",
  {
    variants: {
      variant: {
        ...
        elevated: "shadow-lg border border-border/70",

        // Interactive: Hover con feedback visual (clickeable)
        interactive:
          "border border-border shadow-sm transition-all duration-200 hover:shadow-md hover:border-primary/40 cursor-pointer active:scale-[0.99]",
      },
      padding: {
...
export const CardTitle = component$<{ class?: string }>(
  ({ class: className }) => {
    return (
      <h3 class={cn("text-card-foreground text-xl font-semibold", className)}>
        <Slot />
      </h3>
    );
  },
...
export const CardContent = component$<{ class?: string }>(
  ({ class: className }) => {
    return (
