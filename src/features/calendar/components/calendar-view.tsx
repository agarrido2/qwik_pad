 ...
 *
 * Los estilos de FullCalendar se inyectan aquí vía useStyles$() (lazy, co-localizado).
 * Se usa useStyles$ (NO useStylesScoped$): FullCalendar genera sus propias clases .fc-*
 * en tiempo de ejecución; el scope rompería todos los selectores.
 */
...
import FULLCALENDAR_THEME from "~/assets/css/fullcalendar-theme.css?inline";
...
export const CalendarView = component$<CalendarViewProps>((props) => {
  // Inyecta los estilos de FullCalendar solo cuando este componente es renderizado.
  // Qwik deduplica automáticamente: no se re-inyectan si el componente aparece varias veces.
  useStyles$(FULLCALENDAR_THEME);
  ...
  return (
    <section
      ...
      class="border-border bg-card rounded-lg border p-2 sm:p-4"
    >
      <div ref={containerRef} class="min-h-130 sm:min-h-160" />
    </section>
  );
});
